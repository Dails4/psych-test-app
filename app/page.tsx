...
export default function Home() {
  ...
  const handleShare = async () => {
    ...
    haptic('medium');
    // Включаем полноэкранный лоадер и выводим карточку в DOM (она будет под лоадером)
    setIsSharing(true); 

    try {
      // 500мс — железобетонное время, чтобы любой телефон успел отрисовать карточку
      await delay(500);
  ...
  return (
    <main className="min-h-[100dvh] relative overflow-x-hidden flex flex-col items-center p-6 bg-[#0a0a0f] text-[#f0eeff]">
      ...
      {/* ========================================================================
          ПОЛНОЭКРАННЫЙ ЭКРАН ЗАГРУЗКИ КАРТИНКИ
          ======================================================================== */}
      <AnimatePresence>
        {isSharing && (
          <motion.div 
      ...
      <AnimatePresence mode="wait">
        ...
        {screen === 'result' && resultData && (
          <motion.div key="result" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="z-10 flex flex-col w-full max-w-md h-full flex-1 pt-4 pb-36">
            ...
            {/* ========================================================================
                СКРЫТАЯ КАРТОЧКА: Она физически находится на экране, но под лоадером.
                Браузер отрендерит её со 100% гарантией.
                ======================================================================== */}
            <div className={`fixed top-0 left-0 overflow-hidden pointer-events-none ${isSharing ? 'z-[9998] opacity-100' : 'z-[-99] opacity-0'}`}>
              <div 
                ref={shareCardRef} 
                className="w-[500px] h-[500px] p-8 flex flex-col justify-between text-white"
                style={{
